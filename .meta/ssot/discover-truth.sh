...
DNS_MANAGER="$SCRIPT_DIR/dns/godaddy-dns-manager.sh"
...
# Try envio first, then fall back to .env.godaddy
if command -v envio >/dev/null 2>&1 && envio list 2>/dev/null | grep -q "^godaddy$"; then
    eval "$(envio load godaddy)" 2>/dev/null || warn "Failed to load envio profile 'godaddy'"
else
    source "$PROJECT_ROOT/.env.godaddy" 2>/dev/null || warn "No .env.godaddy found and no envio profile, skipping DNS"
fi

DNS_RECORDS=""
if [[ -n "${GODADDY_API_KEY:-}" ]]; then
    DNS_JSON=$("$DNS_MANAGER" list 2>/dev/null | tail -n +2 | awk 'NF' || echo "")
